package cn.tuyucheng.taketoday.parallel_collectors;
...
public class ParallelCollectorsVirtualThreadsManualTest {
   ...
   public void givenParallelism_whenUsingOSThreads_thenShouldRunOutOfThreads() {
      ...
      var result = timed(() -> Stream.iterate(0, i -> i + 1).limit(parallelProcesses)
            .collect(ParallelCollectors.parallel(ParallelCollectorsVirtualThreadsManualTest::fetchById, toList(), e, parallelProcesses))
            .join());
   ...
   public void givenParallelism_whenUsingVThreads_thenShouldProcessInParallel() {
      ...
      var result = timed(() -> Stream.iterate(0, i -> i + 1).limit(parallelProcesses)
            .collect(ParallelCollectors.parallel(ParallelCollectorsVirtualThreadsManualTest::fetchById, toList()))
            .join());
   ...
   public void givenParallelismAndPCollectors2_whenUsingVThreads_thenShouldProcessInParallel() {
      ...
      var result = timed(() -> Stream.iterate(0, i -> i + 1).limit(parallelProcesses)
            .collect(ParallelCollectors.parallel(ParallelCollectorsVirtualThreadsManualTest::fetchById, toList(), Executors.newVirtualThreadPerTaskExecutor(), Integer.MAX_VALUE))
            .join());
