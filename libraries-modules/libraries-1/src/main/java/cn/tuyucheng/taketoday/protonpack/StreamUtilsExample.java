package cn.tuyucheng.taketoday.protonpack;
...
public class StreamUtilsExample {
   ...
   public void zipAStreamWithIndex() {
      ...
      List<Indexed<String>> zipped = StreamUtils.zipWithIndex(source).toList();
   }
   ...
   public void zipAPairOfStreams() {
      ...
      List<String> zipped = StreamUtils.zip(streamA, streamB, (a, b) -> a + " is for " + b).toList();
   }
   ...
   public void zipThreeStreams() {
      ...
      List<String> zipped = StreamUtils.zip(streamA, streamB, streamC, (a, b, c) -> a + " is for " + b + " " + c).toList();
   }
   ...
   public void windowedStream() {
      ...
      List<List<Integer>> windows = StreamUtils.windowed(integerStream, 2).toList();
      List<List<Integer>> windowsWithSkipIndex = StreamUtils.windowed(integerStream, 3, 2).toList();
      List<List<Integer>> windowsWithSkipIndexAndAllowLowerSize = StreamUtils.windowed(integerStream, 2, 2, true).toList();
   }

   public void groupRunsStreams() {
      Stream<Integer> integerStream = Stream.of(1, 1, 2, 2, 3, 4, 5);

      List<List<Integer>> runs = StreamUtils.groupRuns(integerStream).toList();
   }
