package cn.tuyucheng.taketoday.libraries.jdo.query;
...
public class MyApp {
   ...
   public static void queryUsingJDOQL() {
      Query query = pm.newQuery("SELECT FROM cn.tuyucheng.taketoday.libraries.jdo.query.ProductItem " + "WHERE price < threshold PARAMETERS double threshold");
      List<ProductItem> explicitParamResults = (List<ProductItem>) query.execute(10);

      query = pm.newQuery("SELECT FROM " + "cn.tuyucheng.taketoday.libraries.jdo.query.ProductItem WHERE price < :threshold");
   ...
   public static void queryUsingJPQL() {
      Query query = pm.newQuery("JPQL", "select i from " + "cn.tuyucheng.taketoday.libraries.jdo.query.ProductItem i where i.price < 10" + " and i.status = 'InStock'");
