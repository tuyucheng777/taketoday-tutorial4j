...
public class SSHServerSetup {
   ...
   public static SshServer setupServer() {
      ...
      try {
         ...
         sshd.setPublickeyAuthenticator((username, publicKey, _) -> {
            PublicKeyEntry authorizedKey = authorizedKeys.get(username);
            return authorizedKey != null && authorizedKey.equals(publicKey);
         });

         sshd.setPasswordAuthenticator((username, password, _) -> username.equals(USERNAME) && password.equals(PASSWORD));

         List<SubsystemFactory> subFactList = new ArrayList<>();
