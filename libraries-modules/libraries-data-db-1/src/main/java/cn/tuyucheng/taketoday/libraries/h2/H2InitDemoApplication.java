package cn.tuyucheng.taketoday.libraries.h2;
...
public class H2InitDemoApplication {
   ...
   private static void initDatabaseUsingPlainJDBCWithURL() {
      try (Connection conn = DriverManager.
            getConnection("jdbc:h2:mem:taketoday;INIT=CREATE SCHEMA IF NOT EXISTS taketoday\\;SET SCHEMA taketoday;",
                  "admin",
                  "password")) {
         conn.createStatement().execute("create table users (name VARCHAR(100) NOT NULL, email VARCHAR(100) NOT NULL);");
         System.out.println("Created table users");
         conn.createStatement().execute("insert into users (name, email) values ('Mike', 'mike@taketoday.com')");
   ...
   private static void initDatabaseUsingPlainJDBCWithFile() {
      try (Connection conn = DriverManager.
            getConnection("jdbc:h2:mem:taketoday;INIT=RUNSCRIPT FROM 'src/main/resources/h2init.sql';",
                  "admin",
                  "password")) {
         conn.createStatement().execute("insert into users (name, email) values ('Mike', 'mike@taketoday.com')");
         System.out.println("Added user mike");
