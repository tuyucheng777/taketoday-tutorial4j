package cn.tuyucheng.taketoday.springmodulith.events.externalization.infra;
...
import cn.tuyucheng.taketoday.springmodulith.events.externalization.ArticlePublishedEvent;
...
class ArticlePublishedKafkaProducer {
   ...
   public void publish(ArticlePublishedEvent event) {
      ...
      messageProducer.send("taketoday.articles.published", event);
   }
   ...
   public void publishAsync(ArticlePublishedEvent article) {
      ...
      messageProducer.send("taketoday.articles.published", article.slug(), article);
   }
}