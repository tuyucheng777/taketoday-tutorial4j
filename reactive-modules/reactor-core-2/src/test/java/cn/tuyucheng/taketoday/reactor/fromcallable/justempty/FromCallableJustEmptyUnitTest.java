package cn.tuyucheng.taketoday.reactor.fromcallable.justempty;
...
public class FromCallableJustEmptyUnitTest {
   ...
   public void givenDataAvailable_whenCallingFromCallable_thenLazyEvaluation() {
      ...
      Mono<String> dataFetched = Mono.fromCallable(this::fetchData)
            .doOnSubscribe(_ -> timeTakenForCompletion.set(-1 * System.nanoTime()))
            .doFinally(_ -> timeTakenForCompletion.addAndGet(System.nanoTime()));

      StepVerifier.create(dataFetched)
            .expectNext("Data Fetched")
            .verifyComplete();

      LOGGER.debug("Time Taken to Retrieve Data with Lazy Execution with Subscription");
   ...
   public void givenExceptionThrown_whenCallingFromCallable_thenFromCallableCapturesError() {
      Mono<String> dataFetched = Mono.fromCallable(() -> {
            ...
            .onErrorResume(_ -> Mono.just("COMPLETED"));
   ...
   public void givenDataAvailable_whenCallingJustOrEmpty_thenEagerEvaluation() {
      ...
      Mono<String> dataFetched = Mono.justOrEmpty(fetchData())
            .doOnSubscribe(_ -> timeTakenToReceiveOnCompleteSignalAfterSubscription.set(-1 * System.nanoTime()))
            .doFinally(_ -> timeTakenToReceiveOnCompleteSignalAfterSubscription.addAndGet(System.nanoTime()));
