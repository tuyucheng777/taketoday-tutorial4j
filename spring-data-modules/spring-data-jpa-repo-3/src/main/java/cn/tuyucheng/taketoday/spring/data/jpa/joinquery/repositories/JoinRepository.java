...
public interface JoinRepository extends CrudRepository<ResultDTO, Long> {
   @Query(value = "SELECT new cn.tuyucheng.taketoday.spring.data.jpa.joinquery.DTO.ResultDTO(c.id, o.id, p.id, c.name, c.email, o.orderDate, p.productName, p.price) "
   ...
   @Query(value = "SELECT new cn.tuyucheng.taketoday.spring.data.jpa.joinquery.DTO.ResultDTO_wo_Ids(c.name, c.email, o.orderDate, p.productName, p.price) "
         + " from Customer c, CustomerOrder o ,Product p "
         + " where c.id=o.customer.id "
         + " and o.id=p.customerOrder.id "
         + " and c.id=?1 ")
