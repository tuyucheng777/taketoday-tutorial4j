package cn.tuyucheng.taketoday.clientrx;
...
public class ClientOrchestrationIntegrationTest {
   ...
   public void flowableJavaOrchestrate() throws InterruptedException {
      ...
      userIdFlowable.subscribe((List<Long> employeeIds) -> {
         ...
         Flowable.fromIterable(employeeIds).subscribe(id -> getNameService().register(RxFlowableInvokerProvider.class).resolveTemplate("userId", id).request().rx(RxFlowableInvoker.class).get(String.class) // gotten the name for the given userId
               .doOnError((throwable) -> logger.warn("[FlowableExample] An error has occurred in the username request step {}", throwable.getMessage())).subscribe(userName -> {
                  ...
                  getHashService().register(RxFlowableInvokerProvider.class).resolveTemplate("rawValue", userName + id).request().rx(RxFlowableInvoker.class).get(String.class) // gotten the hash value for userId+username
                        .doOnError((throwable) -> logger.warn(" [FlowableExample] An error has occurred in the hashing request step!", throwable)).subscribe(hashValue -> {
