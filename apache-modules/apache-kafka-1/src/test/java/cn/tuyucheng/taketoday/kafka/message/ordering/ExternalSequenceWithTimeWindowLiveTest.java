package cn.tuyucheng.taketoday.kafka.message.ordering;

import cn.tuyucheng.taketoday.kafka.message.ordering.payload.UserEvent;
import cn.tuyucheng.taketoday.kafka.message.ordering.serialization.JacksonDeserializer;
import cn.tuyucheng.taketoday.kafka.message.ordering.serialization.JacksonSerializer;
...
public class ExternalSequenceWithTimeWindowLiveTest {
   ...
   void givenMultiplePartitions_whenPublishedToKafkaAndConsumedWithExtSeqNumberAndTimeWindow_thenCheckForMessageOrder() throws ExecutionException, InterruptedException {
      ...
      for (long sequenceNumber = 1; sequenceNumber <= 10; sequenceNumber++) {
         ...
         logger.info("User Event ID: {}, Partition : {}", userEvent.getUserEventId(), metadata.partition());
      }
   ...
   private static void processBuffer(List<UserEvent> buffer, List<UserEvent> receivedUserEventList) {
      ...
      buffer.forEach(userEvent -> {
         ...
         logger.info("Processing message with Global Sequence number: {}, User Event Id: {}", userEvent.getGlobalSequenceNumber(), userEvent.getUserEventId());
      });
      buffer.clear();
   }
}