package cn.tuyucheng.taketoday.httpclient.retry;
...
class ApacheHttpClientRetryLiveTest {
   ...
   private void createDefaultApacheHttpClient() {
      this.httpClient = HttpClientBuilder
            .create()
            .addInterceptorFirst((HttpRequestInterceptor) (_, _) -> requestCounter++).build();
   }

   private void createFailingHttpClient() {
      this.httpClient = HttpClientBuilder
            .create()
            .addInterceptorFirst((HttpRequestInterceptor) (_, _) -> requestCounter++)
            .addInterceptorLast((HttpResponseInterceptor) (_, _) -> {
   ...
   private void createHttpClientWithCustomRetryHandler() {
      this.httpClient = HttpClientBuilder
            ...
            .setRetryHandler((_, executionCount, context) -> executionCount < 5 && Objects.equals("GET", ((HttpClientContext) context).getRequest().getRequestLine().getMethod()))
            .build();
   }
