...
public class LdapSetupServlet extends HttpServlet {
   ...
   private void initLdap() {
      try {
         InMemoryDirectoryServerConfig config = new InMemoryDirectoryServerConfig("dc=tuyucheng,dc=com");
         ...
         inMemoryDirectoryServer.importFromLDIF(true,
               new LDIFReader(Objects.requireNonNull(this.getClass().getResourceAsStream("/users.ldif"))));
