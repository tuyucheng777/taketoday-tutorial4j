...
public class InheritanceMappingIntegrationTest {
   ...
   public void givenSubclasses_whenQueryMappedSuperclass_thenOk() {
      MyEmployee emp = new MyEmployee(1, "john", "taketoday");
      session.save(emp);

      assertThat(session.createQuery("from cn.tuyucheng.taketoday.hibernate.pojo.inheritance.Person")
            .getResultList()
            .size()).isEqualTo(1);
   }
   ...
   public void givenSubclasses_whenQueryNonMappedInterface_thenOk() {
      ...
      assertThat(session.createQuery("from cn.tuyucheng.taketoday.hibernate.pojo.inheritance.Item")
            .getResultList()
            .size()).isEqualTo(1);
   }
}
