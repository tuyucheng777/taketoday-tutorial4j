package cn.tuyucheng.taketoday.microstream;
...
class StorageManagerUnitTest {
   ...
   void givenStorageWithStringAsRoot_whenFetchingRoot_thenExpectedStringIsReturned(@TempDir Path tempDir) {
      EmbeddedStorageManager storageManager = StorageManager.initializeStorageWithStringAsRoot(tempDir, "taketoday-demo-1");
      assertThat(storageManager.root()).isEqualTo("taketoday-demo-1");
   ...
   void givenStorageWithCustomTypeAsRoot_whenFetchingRoot_thenCustomTypeWithExpectedValuesIsReturned(@TempDir Path tempDir) {
      EmbeddedStorageManager storageManager = StorageManager.initializeStorageWithCustomTypeAsRoot(tempDir, "taketoday-demo-2");
      RootInstance rootInstance = (RootInstance) storageManager.root();
      assertThat(rootInstance.getName()).isEqualTo("taketoday-demo-2");
   ...
   void givenStorageWithAdditionalObjects_whenLoadingRoot_thenAdditionalObjectsAreSuccessfullyStored(@TempDir Path tempDir) {
      EmbeddedStorageManager storageManager = StorageManager.initializeStorageWithCustomTypeAsRoot(tempDir, "taketoday-demo-3", Arrays.asList(BOOK_ONE, BOOK_TWO));
      RootInstance rootInstance = (RootInstance) storageManager.root();
      assertThat(rootInstance.getName()).isEqualTo("taketoday-demo-3");
   ...
   void givenStorageWithAdditionalObjects_whenLazyLoadingRoot_thenAdditionalObjectsAreSuccessfullyStored(@TempDir Path tempDir) {
      EmbeddedStorageManager storageManager = StorageManager.lazyLoadOrCreateStorageWithCustomTypeAsRoot(tempDir, "taketoday-demo-4", Arrays.asList(BOOK_ONE, BOOK_TWO));
      RootInstanceLazy rootInstance = (RootInstanceLazy) storageManager.root();
      assertThat(rootInstance.getName()).isEqualTo("taketoday-demo-4");
      assertThat(rootInstance.getBooks()).hasSize(2);
      assertThat(rootInstance.getBooks().get(0)).isEqualTo(BOOK_ONE);
      assertThat(rootInstance.getBooks().get(1)).isEqualTo(BOOK_TWO);
      storageManager.shutdown();
   ...
   void givenStorageWithAdditionalObjects_whenRemovingObjectsFromGraph_thenObjectsAreSuccessfullyRemoved(@TempDir Path tempDir) {
      EmbeddedStorageManager storageManager = StorageManager.lazyLoadOrCreateStorageWithCustomTypeAsRoot(tempDir, "taketoday-demo-5", Arrays.asList(BOOK_ONE, BOOK_TWO));
      RootInstanceLazy rootInstance = (RootInstanceLazy) storageManager.root();
      List<Book> books = rootInstance.getBooks();
      books.remove(1);
      storageManager.store(books);
      assertThat(rootInstance.getName()).isEqualTo("taketoday-demo-5");
   ...
   void givenStorageWithAdditionalObjects_whenFilteringCollectionFromGraph_thenStreamsCanBeUsed(@TempDir Path tempDir) {
      EmbeddedStorageManager storageManager = StorageManager.lazyLoadOrCreateStorageWithCustomTypeAsRoot(tempDir, "taketoday-demo-6", Arrays.asList(BOOK_ONE, BOOK_TWO));
      RootInstanceLazy rootInstance = (RootInstanceLazy) storageManager.root();
      List<Book> booksFrom1998 = rootInstance.getBooks().stream()
            .filter(book -> book.getYear() == 1998)
            .collect(Collectors.toList());
      assertThat(rootInstance.getName()).isEqualTo("taketoday-demo-6");
