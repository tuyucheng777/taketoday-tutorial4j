package cn.tuyucheng.taketoday.influxdb;
...
public class InfluxDBConnectionLiveTest {
   ...
   public void whenDatabaseCreatedDatabaseChecksOk() {
      ...
      // Create "taketoday" and check for it
      connection.createDatabase("taketoday");
      assertTrue(connection.databaseExists("taketoday"));

      // Verify that nonsense databases are not there
      assertFalse(connection.databaseExists("foobar"));

      // Drop "taketoday" and check again
      connection.deleteDatabase("taketoday");
      assertFalse(connection.databaseExists("taketoday"));
   }
   ...
   public void whenPointsWrittenPointsExists() throws Exception {
      ...
      String dbName = "taketoday";
      connection.createDatabase(dbName);

      // Need a retention policy before we can proceed
      connection.createRetentionPolicy("defaultPolicy", "taketoday", "30d", 1, true);
      ...
      List<MemoryPoint> memoryPointList = getPoints(connection, "Select * from memory", "taketoday");
   ...
   public void whenBatchWrittenBatchExists() {
      ...
      memoryPointList = getPoints(connection, "Select * from memory order by time desc", "taketoday");
