...
public class ProtobufPackedFieldUnitTest {
   ...
   void whenUnpackedRepeatedProductIds_thenCreateUnpackedOrderAndInvokeRPC() {
      cn.tuyucheng.taketoday.grpc.repeated.UnpackedOrder.Builder unpackedOrderBuilder = cn.tuyucheng.taketoday.grpc.repeated.UnpackedOrder.newBuilder();
      unpackedOrderBuilder.setOrderId(1);
      Arrays.stream(fetchProductIds()).forEach(unpackedOrderBuilder::addProductIds);
      cn.tuyucheng.taketoday.grpc.repeated.UnpackedOrder unpackedOrderRequest = unpackedOrderBuilder.build();

      cn.tuyucheng.taketoday.grpc.repeated.UnpackedOrder unpackedOrderResponse = orderClientStub.createOrder(unpackedOrderRequest);
   ...
   void whenSerializeUnpackedOrderAndPackedOrderObject_thenSizeofPackedOrderObjectIsLess() throws IOException {
      ...
      unpackedOrderBuilder.setOrderId(1);
      Arrays.stream(fetchProductIds()).forEach(unpackedOrderBuilder::addProductIds);
      cn.tuyucheng.taketoday.grpc.repeated.UnpackedOrder unpackedOrder = unpackedOrderBuilder.build();
      ...
      cn.tuyucheng.taketoday.grpc.repeated.PackedOrder.Builder packedOrderBuilder = cn.tuyucheng.taketoday.grpc.repeated.PackedOrder.newBuilder();
      packedOrderBuilder.setOrderId(1);
      Arrays.stream(fetchProductIds()).forEach(packedOrderBuilder::addProductIds);
      cn.tuyucheng.taketoday.grpc.repeated.PackedOrder packedOrder = packedOrderBuilder.build();
   ...
   static String getFolderToWriteObject() {
      final String PROTO_FILE = "proto/repeated.proto";
