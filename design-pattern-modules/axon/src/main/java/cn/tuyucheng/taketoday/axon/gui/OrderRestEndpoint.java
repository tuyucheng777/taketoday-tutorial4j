package cn.tuyucheng.taketoday.axon.gui;
...
import cn.tuyucheng.taketoday.axon.querymodel.OrderQueryService;
import cn.tuyucheng.taketoday.axon.querymodel.OrderResponse;
...
public class OrderRestEndpoint {
   ...
   public CompletableFuture<Void> shipOrder() {
      ...
      return commandGateway.send(new CreateOrderCommand(orderId))
            .thenCompose(_ -> commandGateway.send(new AddProductCommand(orderId, "Deluxe Chair")))
            .thenCompose(_ -> commandGateway.send(new ConfirmOrderCommand(orderId)))
            .thenCompose(_ -> commandGateway.send(new ShipOrderCommand(orderId)));
   }
